...
public class PokemonScreenLAB9 extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		currentPosx = STEP_SIZE*r.nextInt(W/STEP_SIZE); // this should be a random position
		currentPosy = STEP_SIZE*r.nextInt(H/STEP_SIZE); // this should be a random position
		...
		scene.setOnKeyReleased(new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent event) {
				switch (event.getCode()) {
				case UP:
					pikachu.relocate(currentPosx, currentPosy+STEP_SIZE);
					goUp = false;
					break;
				case DOWN:
					goDown = false;
		...
		AnimationTimer timer = new AnimationTimer() {
			...
			public void handle(long now) {
				...
				moveAvatarBy(dx, dy);
				pikachu.relocate(prevPosx,prevPosy);
